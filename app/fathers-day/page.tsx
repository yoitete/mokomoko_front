...
export default function FathersDayPage() {
  ...
  // 父の日関連の投稿をフィルタリング
  const fathersDayPostsWithImages = postsWithImages.filter(
    (post) =>
      (post.tags &&
        post.tags.some(
          (tag) =>
            tag.toLowerCase().includes("父の日") ||
            tag.toLowerCase().includes("fathers") ||
            tag.toLowerCase().includes("father") ||
            tag.toLowerCase().includes("お父さん") ||
            tag.toLowerCase().includes("パパ") ||
  ...
  console.log("Fathers Day posts with images:", fathersDayPostsWithImages);
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-8 relative z-30">
          <h1
            className="text-3xl font-bold text-blue-800 mb-4 !cursor-default relative z-30"
        ...
        {fathersDayPostsWithImages.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            {fathersDayPostsWithImages.map((post, index) => {
